...
def plot_prediction_proba(pred_proba_df):
    n = alt.Chart(pred_proba_df).mark_bar().encode(
        x='Classes',
        y='Probabilities',
        color='Classes'
    )
    ...
    if choice == "Home":
        ...
        with col1:
            ...
            if submit_title_button:
                ...
                pred_proba_df = pd.DataFrame(
                    {'Probabilities': prediction_proba, 'Classes': PIPE_TITLE_vs_TYPE_PREDICTOR.classes_})
        ...
        with col2:
            ...
            if submit_desc_button:
                ...
                pred_proba_df_type = pd.DataFrame(
                    {'Probabilities': PIPE_DESC_vs_TYPE_PREDICTOR.predict_proba([search_desc])[0],
                     'Classes': PIPE_DESC_vs_TYPE_PREDICTOR.classes_})

                pred_proba_df_rating = pd.DataFrame(
                    {'Probabilities': PIPE_DESC_vs_RATING_PREDICTOR.predict_proba([search_desc])[0],
